        ...
        with torch.no_grad():
            ...
            for i in range(len(neighbor_ids)):
                ...
                if comp_frames[idx] is None:
                    ...
                    #Rema:
    pathlib.Path(f"{args.output}").mkdir(exist_ok=True)
    writer = cv2.VideoWriter(f"{args.output}/result.mp4", cv2.VideoWriter_fourcc(*"mp4v"), default_fps, (w, h))
    for f in range(video_length):
        comp = np.array(comp_frames[f]).astype(
            np.uint8)*binary_masks[f] + frames[f] * (1-binary_masks[f])
        pathlib.Path(f"{args.output}/frameresult/").mkdir(exist_ok=True)
        fnameNew=os.path.basename(fnames[f])
        cv2.imwrite(f"{args.output}/frameresult/{fnameNew}",cv2.cvtColor(np.array(comp).astype(np.uint8), cv2.COLOR_BGR2RGB))
        writer.write(cv2.cvtColor(np.array(comp).astype(np.uint8), cv2.COLOR_BGR2RGB))
    writer.release()
    print('Finish in {}'.format(f"{args.output}/result.mp4"))
